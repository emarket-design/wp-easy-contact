...
function wp_easy_contact_enq_custom_css_js() {
	...
	if (!empty($tools['custom_js'])) {
		...
		wp_enqueue_script('wp-easy-contact-custom', add_query_arg(array(
			'wp-easy-contact-js' => 1
		) , $url));
	}
}
/**
...
function wp_easy_contact_print_js() {
	// Only print CSS if this is a stylesheet request
	if (!isset($_GET['wp-easy-contact-js']) || intval($_GET['wp-easy-contact-js']) !== 1) {
		return;
	}
	ob_start();
...
function wp_easy_contact_print_css_js() {
	wp_easy_contact_print_js();
	wp_easy_contact_print_css();
}
add_action('plugins_loaded', 'wp_easy_contact_print_css_js');
/**

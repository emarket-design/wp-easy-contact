...
$container = apply_filters('emd_change_container','emd-wrapper','wp_easy_contact', 'single'); 
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'wp_easy_contact', 'single');
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
		...
		?>
<?php 	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wp_easy_contact', 'single');
	if($has_navigation){
		global $wp_query;
...
<?php 	}
	...
	$show_edit_link = apply_filters('emd_show_single_edit_link',true,'wp_easy_contact');
	if($show_edit_link){

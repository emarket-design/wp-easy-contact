...
$container = apply_filters('emd_change_container','emd-wrapper','wp_easy_contact', 'archive');  
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'wp_easy_contact', 'archive');
...
<?php
	do_action('emd_archive_before_header', 'wp-easy-contact',$post->post_type);
	emd_get_template_part('wp-easy-contact', 'archive', str_replace("_","-",$post->post_type) . '-aheader');
	do_action('emd_archive_after_header', 'wp-easy-contact',$post->post_type);
?>
	</div>
...
<?php
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('wp-easy-contact', 'archive', str_replace("_","-",$post->post_type)); ?>
			</div>
...
<?php
	do_action('emd_archive_before_footer', 'wp-easy-contact',$post->post_type);
	emd_get_template_part('wp-easy-contact', 'archive', str_replace("_","-",$post->post_type) . '-afooter');
	do_action('emd_archive_after_footer', 'wp-easy-contact',$post->post_type);
?>
</div>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wp_easy_contact', 'archive');
	if($has_navigation){
		global $wp_query;
